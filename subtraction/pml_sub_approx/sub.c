...
/* This is the approximate AVX512 Version */
/*
 ...
 * @return none
 */
#define __SUB_N_4_APPROX(result, a, b, b_out)                                 \
    do                                                                        \
    {                                                                         \
 ...
 * @return none
 */
void pml_add_approx_256(limb_t *result, limb_t *a, limb_t *b)
{
    // swap a and b if a < b
    int i = 3;
    do
    ...
    __SUB_N_4_APPROX((result->limbs), (a->limbs), (b->limbs), b_out);
}

void __sub_n_approx(limb_t *result, limb_t *x, limb_t *y)
{
    int n = x->size;
    // swap x and y if x < y
    int i = n - 1;
...
void pml_add_approx(limb_t *result, limb_t *a, limb_t *b)
{
    int n = a->size;

    if (n <= 4)
    {
        pml_add_approx_256(result, a, b);
    }
    else
    {
...
void run_perf_test()
{
    ...
    double t;
    RECORD_RDTSC(t, pml_add_approx(a, b, s));
...
void run_benchmarking_test(int test_case, int measure_type)
{
    ...
    for (int iter_count = 0; iter_count < (ITERATIONS / ITERATIONS); ++iter_count)
    {
        ...
        case 0:             // RDTSC
            ...
            TIME_RDTSC(time_taken, pml_add_approx(a, b, s));
            printf("done\n");
        ...
        case 1:             // Timespec
            ...
            TIME_TIMESPEC(time_taken, pml_add_approx(a, b, s));
        ...
        case 2:             // Rusage
            ...
            // calibrate the time
            TIME_RUSAGE(time_taken, pml_add_approx(s, a, b));
