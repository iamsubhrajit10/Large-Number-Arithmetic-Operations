...
#define __SUB_N_4(result, a, b, b_out)                                                               \
    ...
    {                                                                                                \
        ...
        b_out = b_mask >> 3;                                                                         \
        b_mask <<= 1;                                                                                \
        __m256i result_vec_new = _mm256_mask_add_epi64(result_vec, b_mask, result_vec, AVX256_MASK); \
...
void pml_sub_n(limb_t *result, limb_t *a, limb_t *b)
{
    int n = a->size;
...
void run_perf_test()
{
    ...
    double t;
    RECORD_RDTSC(t, pml_sub_n(s, a, b));
...
void run_benchmarking_test(int test_case, int measure_type)
{
    ...
    for (int iter_count = 0; iter_count < (ITERATIONS / ITERATIONS); ++iter_count)
    {
        ...
        case 0:             // RDTSC
            ...
            TIME_RDTSC(time_taken, pml_sub_n(s, a, b));
            // printf("done\n");
        ...
        case 1:             // Timespec
            ...
            TIME_TIMESPEC(time_taken, pml_sub_n(s, a, b));
        ...
        case 2:             // Rusage
            ...
            // calibrate the time
            TIME_RUSAGE(time_taken, pml_sub_n(s, a, b));
