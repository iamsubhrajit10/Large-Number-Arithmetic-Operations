...
#define LIMB_SIZE 9       // Number of digits in each limb
...
aligned_uint64 LIMB_DIGITS = 10000000000000000000; // 10^19, used for borrow-propagation, as we're using 64-bit integers
...
inline void sub_n(uint64_t *a, uint64_t *b, uint64_t **result_ptr, int n, int *result_size) __attribute__((always_inline));
