...
// files to write for entry-exit rdtsc values
char entry_exit_name[256];

FILE *entry_exit_file;

inline void sub_n(uint32_t *a, uint32_t *b, uint32_t **result_ptr, int n, int *result_size)
{
    unsigned long long entry_rdtsc, exit_rdtsc;

    // warm up for rdtscp
    warmup_rdtsc();

    // measure the rdtsc for entry
    entry_rdtsc = measure_rdtsc_start();
    ...
    *result_size = *result_size - i;
    // measure the rdtsc for exit
    exit_rdtsc = measure_rdtscp_end();
    if (exit_rdtsc < entry_rdtsc)
    {
        perror("Error in rdtsc\n");
        exit(EXIT_FAILURE);
    }
    // write the ticks
    fprintf(entry_exit_file, "%llu\n", exit_rdtsc - entry_rdtsc);
}
...
int main(int argc, char *argv[])
{
    ...
    // open the file to write the entry and exit rdtsc values
    snprintf(entry_exit_name, sizeof(entry_exit_name), "entry_exit_rdtsc_%d_%d.csv", NUM_BITS, CORE_NO);
    entry_exit_file = fopen(entry_exit_name, "w");
    run_tests();

    // close the file
    fclose(entry_exit_file);
    return 0;
}
