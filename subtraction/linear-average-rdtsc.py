...
def plot_linear_curve(averages, labels, output_path, test_case):
    ...
    plt.xlabel('Bit Size')
    plt.ylabel('Average RDTSC Value')
    plt.legend(loc='best')
    plt.grid(True)
...
def process_directory(subdir, gmp_dir, averages, test_case):
    ...
    gmp_input_dir = os.path.join(current_path, gmp_dir, 'experiments/results/rdtsc_measurements/')
...
def main():
    ...
    for test_case in test_cases:
        ...
        output_path = os.path.join(os.getcwd(), 'linear-curves', 'rdtsc',f'average_values_comparison_{test_case}.png')
