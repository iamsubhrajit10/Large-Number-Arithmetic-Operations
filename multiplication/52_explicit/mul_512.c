...
void limb_mul_n_256(limb_t *a, limb_t *b, uint64_t *res_lo, uint64_t *res_hi)
{
    ...
    // Combine multiple additions into single operations
    uint64_t temp = res_lo[22] + res_lo[23];
    res_hi[8] = temp & 0xFFFFFFFFFFFFF;
    uint carry = temp >> 52;

    temp = res_lo[19] + res_lo[20] + res_lo[21] + carry;
    res_hi[7] = temp & 0xFFFFFFFFFFFFF;
    carry = temp >> 52;

    temp = res_lo[15] + res_lo[16] + res_lo[17] + res_lo[18] + carry;
    res_hi[6] = temp & 0xFFFFFFFFFFFFF;
    carry = temp >> 52;

    temp = res_lo[10] + res_lo[11] + res_lo[12] + res_lo[13] + res_lo[14] + carry;
    res_hi[5] = temp & 0xFFFFFFFFFFFFF;
    carry = temp >> 52;

    temp = res_lo[6] + res_lo[7] + res_lo[8] + res_lo[9] + carry;
    res_hi[4] = temp & 0xFFFFFFFFFFFFF;
    carry = temp >> 52;

    temp = res_lo[3] + res_lo[4] + res_lo[5] + carry;
    res_hi[3] = temp & 0xFFFFFFFFFFFFF;
    carry = temp >> 52;

    temp = res_lo[1] + res_lo[2] + carry;
    res_hi[2] = temp & 0xFFFFFFFFFFFFF;
    carry = temp >> 52;

    temp = res_lo[0] + carry;
    res_hi[1] = temp & 0xFFFFFFFFFFFFF;
    res_hi[0] += (temp >> 52);
    // printf("Done with the multiplication\n");
}

// use karatsuba multiplication for 512-bit numbers, and base case as 256-bit AVX Urdhva

void limb_mul_n_512(limb_t *a, limb_t *b, limb_t *res)
{
    uint64_t *num1 = a->limbs;
    uint64_t *num2 = b->limbs;
    int n = a->size;
    if (n == 5)
    {
        limb_mul_n_256(a, b, res->limbs, res->limbs + 4);
        return;
    }

    uint64_t *num1_hi = num1;
    uint64_t *num1_lo = num1 + (n >> 1);

    uint64_t *num2_hi = num2;
    uint64_t *num2_lo = num2 + (n >> 1);

    uint64_t *res_lo_0 = res->limbs;
    uint64_t *res_lo_1 = res->limbs + (n >> 1) * sizeof(uint64_t);
    uint64_t *res_lo_2 = res->limbs + (n >> 1) * sizeof(uint64_t) * 2;

    uint64_t *res_hi_0 = res->limbs + (n >> 1) * sizeof(uint64_t) * 3;
    uint64_t *res_hi_1 = res->limbs + (n >> 1) * sizeof(uint64_t) * 4;
    uint64_t *res_hi_2 = res->limbs + (n >> 1) * sizeof(uint64_t) * 5;

    // perform mul(num1_hi, num2_hi)
    limb_mul_n_256(a, b, res_lo_0, res_hi_0);

    // perform mul(num1_hi, num2_lo)
    limb_mul_n_256(a, b, res_lo_1, res_hi_1);

    // perform mul(num1_lo, num2_lo)
    limb_mul_n_256(a, b, res_lo_2, res_hi_2);
}

void test_512_mul(int test_case, int measure_type)
{
    printf("Trying to run 512 test\n");
    ...
    for (int iter_count = 0; iter_count < 10; ++iter_count)
    {
        ...
        limb_t *res = limb_t_alloc(5 * n * sizeof(uint64_t));
        ...
        case 0:             // RDTSC
            ...
            TIME_RDTSC(time_taken, limb_mul_n_512(a, b, res));
            printf("done\n");
        ...
        case 1:             // Timespec
            ...
            TIME_TIMESPEC(time_taken, limb_mul_n_512(a, b, res));
        ...
        case 2:             // Rusage
            ...
            // calibrate the time
            TIME_RUSAGE(time_taken, limb_mul_n_512(a, b, res));
...
int main(int argc, char *argv[])
{
    ...
    test_512_mul(test_case, measure_type);
...
void run_correctness_test(int test_case)
{
    ...
    for (int i = 0; i < ITERATIONS; i++)
    {
        ...
        limb_mul_n_256(a, b, res_lo->limbs, res_hi->limbs);
...
void run_benchmarking_test(int test_case, int measure_type)
{
    ...
    for (int iter_count = 0; iter_count < 10; ++iter_count)
    {
        ...
        case 0:             // RDTSC
            ...
            TIME_RDTSC(time_taken, limb_mul_n_256(a, b, res_lo->limbs, res_hi->limbs));
            printf("done\n");
            printf("Calibrated time: %f microseconds\n", time_taken);
        ...
        case 1:             // Timespec
            ...
            TIME_TIMESPEC(time_taken, limb_mul_n_256(a, b, res_lo->limbs, res_hi->limbs));
        ...
        case 2:             // Rusage
            ...
            // calibrate the time
            TIME_RUSAGE(time_taken, limb_mul_n_256(a, b, res_lo->limbs, res_hi->limbs));
