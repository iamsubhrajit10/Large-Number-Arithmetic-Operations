...
// files to write first, second loop and other areas
char first_loop_name[256];
char second_loop_name[256];
char other_name[256];

FILE *first_loop_file;
FILE *second_loop_file;
FILE *other_file;

inline void sub_n(uint32_t *a, uint32_t *b, uint32_t **result_ptr, int n, int *result_size)
{
    unsigned long long start_rdtsc_first, end_rdtsc_first;
    unsigned long long start_rdtsc_second, end_rdtsc_second;
    unsigned long long start_rdtsc_other, end_rdtsc_other;

    unsigned long long first_loop_rdtsc = 0;
    unsigned long long second_loop_rdtsc = 0;
    unsigned long long other_rdtsc = 0;

    // warm up for rdtscp
    warmup_rdtsc();

    // measure the rdtsc for the other areas
    start_rdtsc_other = measure_rdtsc_start();
    ...
    // measure the rdtsc for the other areas
    end_rdtsc_other = measure_rdtscp_end();
    other_rdtsc += (end_rdtsc_other - start_rdtsc_other);

    // measure the rdtsc for the first loop
    start_rdtsc_first = measure_rdtsc_start();
    ...
    // measure the rdtsc for the first loop
    end_rdtsc_first = measure_rdtscp_end();
    first_loop_rdtsc += (end_rdtsc_first - start_rdtsc_first);
    ...
    // measure the rdtsc for the second loop
    start_rdtsc_second = measure_rdtsc_start();
    ...
    // measure the rdtsc for the second loop
    end_rdtsc_second = measure_rdtscp_end();
    second_loop_rdtsc += (end_rdtsc_second - start_rdtsc_second);
    ...
    *result_size = *result_size - i;
    ...
    // write the rdtsc values to the files
    fprintf(first_loop_file, "%llu\n", first_loop_rdtsc);
    fprintf(second_loop_file, "%llu\n", second_loop_rdtsc);
    fprintf(other_file, "%llu\n", other_rdtsc);
}
...
int main(int argc, char *argv[])
{
    ...
    snprintf(first_loop_name, sizeof(first_loop_name), "first_loop_%d_%d.csv", NUM_BITS, CORE_NO);
    snprintf(second_loop_name, sizeof(second_loop_name), "second_loop_%d_%d.csv", NUM_BITS, CORE_NO);
    snprintf(other_name, sizeof(other_name), "other_%d_%d.csv", NUM_BITS, CORE_NO);

    // open the files using fopen, in append mode
    first_loop_file = fopen(first_loop_name, "w");
    second_loop_file = fopen(second_loop_name, "w");
    other_file = fopen(other_name, "w");

    run_tests();

    // close the files
    fclose(first_loop_file);
    fclose(second_loop_file);
    fclose(other_file);
