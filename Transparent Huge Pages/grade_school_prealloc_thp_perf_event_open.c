...
// function to flush the cache of a given pointer and size
...
int main() {
    ...
    snprintf(CSV_FILENAME, sizeof(CSV_FILENAME), "experiment_grade_school_multiplication_results_peo_thp_%d.csv", NUMBER_OF_BITS);
    ...
    int *results_space;
    err = posix_memalign((void **)&results_space, HPAGE_SIZE, (NUM_DIGITS/2)*(2*(sample_length+1) + 1) * sizeof(int));
    if (err != 0) {
        perror("posix_memalign results_space");
        exit(EXIT_FAILURE);
    }
   ...
   // Open a file for writing
    char binary_name[] = "perf_peo_grade_school_thp"; // replace with actual binary name
